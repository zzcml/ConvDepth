...
class Trainer:
    def __init__(self, options):
        ...
        self.models["encoder"] = networks.convnext_v2()
        self.models["encoder"].to(self.device)
        self.parameters_to_train = self.parameters_to_train + list(self.models["encoder"].parameters())

        self.models["depth"] = networks.ConvDecoder(
            self.models["encoder"].num_ch_enc, self.opt.scales)
        self.models["depth"].to(self.device)
        self.parameters_to_train = self.parameters_to_train + list(self.models["depth"].parameters())
        ...
        if self.use_pose_net:
            if self.opt.pose_model_type == "separate_resnet":
                ...
                self.parameters_to_train = self.parameters_to_train + list(self.models["pose_encoder"].parameters())
            ...
            self.parameters_to_train = self.parameters_to_train + list(self.models["pose"].parameters())
        ...
        self.model_optimizer = optim.Adam([
            {'params': self.models["encoder"].parameters(), 'lr': self.opt.learning_rate},
            {'params': self.models["depth"].parameters(), 'lr': 6 * self.opt.learning_rate},
            {'params': self.models["pose"].parameters(), 'lr': 10 * self.opt.learning_rate},
            {'params': self.models["pose_encoder"].parameters(), 'lr': 10 * self.opt.learning_rate}],
    ...
    def generate_images_pred(self, inputs, outputs):
        ...
        """
        for scale in self.opt.scales:
            disp = outputs[("disp", scale)]   ##Parallax
            ...
            _, depth = disp_to_depth(disp, self.opt.min_depth, self.opt.max_depth)  ##depth
            ...
            for i, frame_id in enumerate(self.opt.frame_ids[1:]):
                ...
                cam_points = self.backproject_depth[source_scale](
                    depth, inputs[("inv_K", source_scale)])   # 2d to 3d
                pix_coords = self.project_3d[source_scale](
                    cam_points, inputs[("K", source_scale)], T) #3d to 2d
    ...
    def compute_reprojection_loss_0(self, pred, target):
        """Computes reprojection loss between a batch of predicted and target images
        """
        abs_diff = torch.abs(target - pred)
        l1_loss = abs_diff.mean(1, True)
        a=(target-pred).permute(0,3,2,1)
        rf=self.AdaptiveImageLossFunction.lossfun(a).permute(0,3,2,1)
        afloss=rf.mean(1,True)

        if self.opt.no_ssim:
            reprojection_loss = l1_loss
        else:
            ssim_loss = self.ssim(pred, target).mean(1, True)
            reprojection_loss = 0.85 * ssim_loss + 0.15 * afloss
            # reprojection_loss =ssim_loss

        return reprojection_loss

    def compute_reprojection_loss_1(self, pred, target,sd):
        """Computes reprojection loss between a batch of predicted and target images
        """
        abs_diff = torch.abs(target - pred)
        l1_loss = abs_diff.mean(1, True)
        ...
        if self.opt.no_ssim:
            ...
            ssim_loss = self.ssim(pred, target).mean(1, True)
            reprojection_loss = 0.85 * ssim_loss + 0.15 * sd

        return reprojection_loss

    def Sd(self,pre_0,pre, epoach, thre=10):
        if epoach < 10:
            Ladb = epoach / thre
        else:
            Ladb = 1
        sd_loss=torch.abs(pre_0-pre)
        # sd_loss=torch.log(abs(pre_0-pre)+1)
        sd_loss=sd_loss/3
        sd_loss=Ladb*sd_loss
    ...
    def compute_losses(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            """ rubust and self-distillation loss"""
            if scale==0:
                for frame_id in self.opt.frame_ids[1:]:
                    pred = outputs[("color", frame_id, scale)]
                    reprojection_losses.append(self.compute_reprojection_loss_0(pred, target))
                    pre_0.append(pred)

            else:
                for frame_id in self.opt.frame_ids[1:]:
                    pred = outputs[("color", frame_id, scale)]
                    sd_loss=self.Sd(pre_0[0],pred,self.epoch)
                    reprojection_losses.append(self.compute_reprojection_loss_1(pred, target,sd_loss))
            """ rubust and self-distillation loss"""

            """ L1 loss"""
            # for frame_id in self.opt.frame_ids[1:]:
            #     pred = outputs[("color", frame_id, scale)]
            #     reprojection_losses.append(self.compute_reprojection_loss(pred, target))  # warping source  --   source
            """ L1 loss"""

            """ Robust loss"""
            # for frame_id in self.opt.frame_ids[1:]:
            #     pred = outputs[("color", frame_id, scale)]
            #     reprojection_losses.append(self.compute_reprojection_loss_0(pred, target))  # warping source  --   source
            """ Robust loss"""
            ...
            if not self.opt.disable_automasking:
                ...
                for frame_id in self.opt.frame_ids[1:]:
                    pred = inputs[("color", frame_id, source_scale)]  #target -- source
                    identity_reprojection_losses.append(
                        self.compute_reprojection_loss_0(pred, target))
            ...
            if not self.opt.disable_automasking:
                ...
                combined = torch.cat((identity_reprojection_loss, reprojection_loss), dim=1)  #comparasion between warping and no warping
            else:
            ...
            if combined.shape[1] == 1:
                ...
                to_optimise, idxs = torch.min(combined, dim=1)   #mini reprojection error

            if not self.opt.disable_automasking:
                outputs["identity_selection/{}".format(scale)] = (
                    idxs > identity_reprojection_loss.shape[1] - 1).float()  # 为了可视化，indicates that one of the reprojection_loss channels was smallest for this pixel.
